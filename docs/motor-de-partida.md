...
### 5.30. Correção crítica: vaga de Sul-Americana sumindo por completo em temporadas seguintes (implementado)

Bug urgente relatado pelo jogador: "meu clube se classificou para a sulamericana - porém, no ano seguinte, a sudamericana não apareceu nos campeonatos do meu time". Rastreado com scripts de dado real (mesmo padrão de sempre) simulando várias temporadas seguidas — a causa raiz **não era o clube do jogador especificamente, era uma falha estrutural que podia derrubar Libertadores+Sul-Americana inteiras pra QUALQUER jogador** assim que a troca de vaga continental de Chile ou Bolívia (`career/mundo-persistente.ts` `calcularMudancasContinentais`, seção 5.26) disparava.

- **Causa raiz**: `sulamericana.json`/`libertadores.json` usam etapas pré-classificatórias com `entrantes` explícitos, fixos (`simulation/incremental.ts` `derivarCortePreClassificatorio` — deriva "quantos clubes entram direto na fase de grupos" subtraindo esses `entrantes` do total). Alguns clubes chilenos/bolivianos/brasileiros (`ohiggins`, `bolivar`, `botafogo`) estão cadastrados em AMBAS as competições — uma vez como entrada direta (Sul-Americana) e de novo dentro de uma etapa pré-classificatória (Libertadores). `calcularMudancasContinentais` não sabia disso: ao trocar a fatia de um país, podia acabar removendo justamente um desses clubes com papel fixo — o total do país continuava batendo (troca é sempre N-por-N), mas o EQUILÍBRIO "quantos entram direto vs quantos vêm de pré-classificatória" quebrava, e `derivarCortePreClassificatorio` não achava mais o corte exato → **lança erro** → Libertadores E Sul-Americana inteiras viram `erro` na temporada e SOMEM da lista de competições de todo mundo, não só de quem tinha acabado de ganhar a vaga.
- **Fix**: `calcularMudancasContinentais` ganhou um parâmetro `clubesProtegidos?: Set<string>` — clubes com papel fixo de pré-classificatória nunca contam na fatia "atual" de um país nem podem ser escolhidos como novo entrante; a troca por vaga passa a mexer só nos clubes "diretos" de verdade. `useTemporada.ts` computa esse conjunto a partir de `formato.mata_mata.etapas[].entrantes` de Libertadores + Sul-Americana combinadas.
- **Dados ajustados pra bater com a nova contagem "só diretos"** (ver `docs/dados-a-verificar.md`): `chile_primera.json`/`bolivia_primera.json` `vaga_sulamericana` volta de 5→4 (o "5" era só a contagem ANTIGA, incorreta, que incluía o clube protegido; 4 bate com o critério pesquisado uma vez que o protegido é excluído — nenhuma mudança de dado nova, na real); `brasileirao_serie_a.json` `vaga_sulamericana` 7→6 (Botafogo é o clube protegido brasileiro — só as outras 6 vagas são preenchidas por posição na tabela temporada a temporada).
- **Validado**: 3 testes novos em `tests/career/mundo-persistente.test.ts` (clube protegido excluído da fatia atual e nunca trocado; sem a proteção a troca seria pulada por contagem não bater; clube protegido nunca é escolhido como novo entrante). Scripts ad-hoc com dado real simularam 6 temporadas seguidas de uma carreira brasileira: zero erros em Libertadores/Sul-Americana em todas as 6, com trocas de composição acontecendo normalmente pra Brasil/Chile/Bolívia (confirmado repetindo a simulação várias vezes, já que o resultado depende de sorteio). 527 testes passando, `npx tsc --noEmit`/`cd web && npx tsc -b` limpos.

### 5.31. Correção do formato do Paulistão A1: classificação por pote + chaveamento do mata-mata (implementado)

Bug relatado pelo jogador jogando com o São Paulo: "ainda não estão separados em 4 grupos, está uma tabela única na classificação - eu fiquei em 5 e não fui pro mata-mata". `paulistao_a1.json` já modelava `fase_suica.classificacao_por_pote` corretamente (4 potes de 4, top-2 de cada avança), mas a UI (`web/src/features/temporada/TelaDeTemporada.tsx` `PainelClassificacao`) sempre desenhava uma tabela única com destaque por posição NUMÉRICA fixa (`FaixasDeDestaqueDaTabela`/`classeFaixaDaLinha`, `useTemporada.ts`) — mesma categoria de bug já visto antes na Série D: um esquema de destaque puramente posicional não consegue representar corretamente um recorte de classificação NÃO contíguo (top-2 de cada um de 4 potes intercalados na tabela geral).

- **Fix de UI**: novo componente `TabelaPorPotes` (`TelaDeTemporada.tsx`) — quando a competição tem `classificacao_por_pote`, a Classificação renderiza 4 sub-tabelas ("Pote 1"–"Pote 4"), cada uma já filtrada e com destaque de top-2 correto dentro do próprio pote, em vez de uma tabela única. `useTemporada.ts` ganhou `potesFaseSuicaPorCampeonato` (usa `construirPotePorTime`, já existente em `simulation/swiss.ts`) exposto pelo hook e passado adiante até `PainelClassificacao`.
- **Chaveamento do mata-mata**: pesquisa pública (CNN Brasil, Itatiaia) descreve um chaveamento "olímpico" cross-pote pela classificação geral (1ºx8º, 2ºx7º etc.) nas quartas do Paulistão real — implementação inicial seguiu essa fonte (`paresSeededPorClassificacaoGeral`). Perguntado diretamente, porém, o jogador **confirmou a regra que já tinha descrito antes**: 1º e 2º colocados do MESMO pote se enfrentam (mandante = 1º), não o cruzamento da fonte pública. **Divergência não resolvida**: pode ser edição diferente do formato coberta pelas matérias, ou o jogador descrevendo de memória um regulamento antigo — mantido conforme a confirmação direta dele (`paresPorPoteDaFaseSuica`, `simulation/incremental.ts`), com a fonte pública documentada aqui como ressalva.
- Quartas e semifinal em jogo único, final em ida e volta (`paulistao_a1.json` `formato.mata_mata.etapas`, novo mecanismo genérico `etapasParaMataMataSimples` que lê `etapas[]` per-etapa quando presente em vez do único `ida_e_volta` compartilhado de `fases[]`) — confirmado pelo jogador, bate com quartas/semis já pesquisadas como jogo único nas fontes públicas (só a fonte diverge no formato da final, que uma delas descreve como jogo único também).
- **Não modelado** (pendência): as semifinais reais do Paulistão reordenariam os 4 classificados por pontuação acumulada (pontos da fase suíça + bônus da quarta) antes de formar os pares — como a regra vigente aqui é por pote (não pelo seed da fonte pública), essa reordenação não se aplica; mantido como não implementado.
- **Validado**: novo teste em `tests/simulation/incremental.test.ts` (chaveamento das quartas forma pares disjuntos válidos), script ad-hoc com dado real simulando o Paulistão A1 inteiro confirma pares 1º/2º do mesmo pote, quartas/semis em jogo único e final em ida-e-volta. 528 testes passando, `npx tsc --noEmit`/`cd web && npx tsc -b` limpos.

### 5.32. Lote de correções: calendário aleatório, rebaixamento multi-divisão, pênaltis reais, idade, rodada/fase na tela de partida, treino descontinuado (implementado)

Lote de 7 pedidos do jogador numa sessão só, priorizados como bug de lógica de jogo primeiro, depois dado/formato, depois UX, depois a mudança de design mais invasiva por último:

- **Calendário aleatório por temporada**: `gerarConfrontosPontosCorridos` (`simulation/season.ts`) era puramente determinístico pela ORDEM do array `times` — como essa ordem nunca mudava (vem direto do arquivo de dado estático), a rodada 1 repetia sempre o mesmo confronto toda temporada (bug relatado: "a rodada 1 sempre é contra o mesmo time"). Ganhou um `random?` opcional que embaralha uma cópia de `times` antes do método do círculo; `simulation/incremental.ts` `construirPassos` embaralha `campeonato.times` uma vez por temporada (EXCETO formatos `fase_suica`, cuja ordem É o sorteio real dos potes — embaralhar ali destruiria a seeding, ver `swiss.ts` `dividirEmPotes`). `fase_grupos` não é afetado pela ressalva: `dividirEmGruposPorForca` já reordena por `ratings`, não pela ordem de entrada.
- **Rebaixamento/acesso em estaduais multi-divisão**: bug mais amplo do que o relatado ("não estou conseguindo validar se o rebaixamento está funcionando") — TODOS os estaduais com 2+ divisões (Paulistão A1-A4, Carioca, Mineiro, Baiano, Pernambucano, Gauchão) tinham o mecanismo de `career/mundo-persistente.ts` quebrado, cada um por um motivo técnico diferente (detalhado em `docs/regras-competicoes.md`). Generalizado: `fase_suica` passa a expor `tabelaFinal` (tabela geral, válida só pra REBAIXAMENTO); `turno`+`returno` (Carioca A) soma as 2 tabelas num `tabelaFinal`; `semifinalistas` deixou de exigir bater o tamanho EXATO de `acesso_proxima_divisao` — virou uma lista ORDENADA por eliminação (`classificacaoPorEliminacao`, substitui a antiga `semifinalistasDaFase`), consumida via `slice(0, acesso)`. Paulistão A2/A3 ganharam `acesso_proxima_divisao` que faltava (ajustado pra CASAR com o `rebaixamento_proxima_divisao` do vizinho de cima, mesma lógica de simetria já usada pra Série C/D na seção 5.27 — sem isso o A1/A2 drenariam 1-2 times/temporada até quebrar a validação de contagem exata da fase suíça).
- **Pênaltis reais**: `simulation/knockout.ts` `resolverConfronto` decidia o vencedor do agregado empatado com uma "moeda ponderada pelo rating", sem simular cobrança nenhuma. Nova `simularDisputaDePenaltis` — 5 cobranças por time (parando cedo se matematicamente decidido, regra padrão de futebol) + morte súbita, conversão base ~78% ajustada por rating; quando o clube do JOGADOR está envolvido, `frieza` do cobrador (se meia/atacante) ou `reflexos` do goleiro adversário pesam na probabilidade (Camada 2, sem precisar de UI de cobrança individual). `ResultadoConfrontoMataMata.penaltis` novo campo com o placar da disputa.
- **Reformulação do placar de ida/volta**: ida mostrada normal ("Santos 1-0 Palmeiras"); volta anotada com o placar da ida entre parênteses ao lado de cada time ("(1) Santos x Palmeiras (0)", `LinhaDeConfrontoMataMata` nova prop `golsIdaMandante`/`golsIdaVisitante`); placar de pênaltis mostrado no resultado da rodada, no bracket (`ChaveamentoDaCompeticao`) e no feed quando `decididoNosPenaltis`.
- **Idade do jogador**: `IDADE_INICIAL_PADRAO` (`career/Player.ts`) 18→17; exibida no cabeçalho da tela de temporada (antes ausente).
- **Rodada/fase na tela de partida**: o dado já existia no motor (`Confronto.rodada`, etapa de mata-mata) mas só chegava à UI DEPOIS da partida (resultado da rodada) — `ContextoConfronto` (`simulation/match.ts`) ganhou `rodada?`/`etapa?`, propagados por TODOS os pontos que chamam `resolverPartida` (pontos corridos em `season.ts`/`incremental.ts`; mata-mata em `knockout.ts` `resolverConfronto`, que ganhou um parâmetro opcional `etapa` threaded por todos os 4 call sites) até `career/career-loop.ts` `ContextoPartidaDoJogador`/`ContextoPartidaDoJogadorSemanal`, exibido em `PainelPrePartida` e `PainelPartidaAoVivo`.
- **Treino descontinuado, XP só de partida**: removido o sistema de escolha de foco de treino inteiro (`FocoDeTreino`, `ATRIBUTOS_POR_FOCO`, `xpDeSessaoDeTreino`, `aplicarGanhoDeTreino`, `escolherFocoDeTreino`/`onTreinoResolvido` em `career-loop.ts`, `PromptFoco`/`focoAutomatico`/"treino rápido" na UI) — XP vem exclusivamente de `aplicarDesempenhoPartida` (já existia, sem mudança). Moral recupera passivamente (`MORAL_RECUPERADA_PASSIVAMENTE_POR_PERIODO = 3`, bem menor que o antigo `MORAL_RECUPERADA_NO_DESCANSO = 8` de propósito — antes só recuperava quando o jogador ABRIA MÃO de treinar, agora é automático todo período, precisa ser mais modesto pra não inflar moral de graça). `escolherDistribuicaoDePontos` deixa de ser chamado 1x por período e passa a disparar imediatamente a cada level-up (em `jogarTemporada`/`jogarTemporadaSemanal`, logo após `onNivelAlcancado`), reaproveitando o mesmo prompt de alocação de pontos que já existia.
- **Validado**: suíte completa (536+ testes, incluindo novos testes de pênaltis com PRNG seedado, rodada/etapa em `escolherModoDePartida`, recuperação passiva de moral, ordem `onNivelAlcancado`→`escolherDistribuicaoDePontos`), `npx tsc --noEmit`/`cd web && npx tsc -b`/`cd web && npm run build` limpos.

### 5.33. Taças reais mapeadas pra campeonatos (imagens locais) + 3 títulos próprios da Argentina (implementado)

Pedido do usuário: "src/data/trofeus tem a imagem de alguns dos troféus configurados, eu quero que vc mapeie eles pros campeonatos" — 12 SVGs curados manualmente pelo jogador. Primeira vez que `taca_url` aponta pra um arquivo LOCAL em vez de URL externa (ver `docs/dados-a-verificar.md` seção de taças pra convenção completa) — cópia em `web/public/trofeus/<slug>.svg` (Vite só serve estático de `public/`), `taca_url` vira `/trofeus/<slug>.svg`.

- **7 mapeamentos diretos, sem ambiguidade**: Copa do Brasil, Libertadores, Sul-Americana, Brasileirão Série A-D — substituíram os `taca_url` externos do TheSportsDB (`scripts/buscar-tacas.ts`).
- **Argentina — mudança de regra, não só de dado**: o jogador forneceu 3 imagens (Apertura/Clausura/Tabla Anual), mas o motor só premiava UM título por competição (`ctx.campeao`, sempre a Tabla Anual — Apertura/Clausura eram tabelas intermediárias somadas, nunca títulos reais). Corrigido: `simulation/engine.ts` `ResultadoCampeonatoSimples`/`receitaArgentina` e `simulation/incremental.ts` `ContextoDePrograma`/`CompeticaoIncremental`/`passosTurnoRetornoSomado` ganharam `tituloApertura`/`tituloClausura` — líder de CADA meio-turno isolado, título próprio de verdade (nome deliberadamente diferente do `ctx.campeaoApertura`/`campeaoClausura` que Uruguai/Peru/Colômbia/Venezuela já usavam internamente só como semente de playoff, pra não confundir os dois conceitos nem criar título fantasma nesses países). `CampeonatoNacional` ganhou `taca_apertura_url`/`taca_clausura_url` (`taca_url` continua sendo só a Tabla Anual/campeão geral). `web/src/features/temporada/useTemporada.ts` `TituloDeCarreira` ganhou `subtitulo?: "apertura" | "clausura"` — um clube pode ganhar até 3 títulos na MESMA temporada da mesma competição agora (`novosTitulos` virou `flatMap`, não `map`). `tacaPorCampeonato` virou `Map<string, InfoTacasDaCompeticao>` (`{ principal, apertura, clausura }`) em vez de `Map<string, string | undefined>` — a sala de troféus escolhe a taça certa por `subtitulo` e rotula "Apertura —"/"Clausura —" quando aplicável.
- **Efeito colateral correto, não bug**: `passosTurnoRetornoSomado` também é a receita do Paraguai (mesmo formato `turno`+`returno` somado, sem final) — Paraguai passa a ter Apertura/Clausura como títulos reais também, o que é fielmente correto (mesmo mecanismo real do futebol paraguaio), só que sem taça própria cadastrada ainda (cai no ícone genérico até alguém fornecer imagem).
- **Fora do escopo, por decisão do usuário**: `copa argentina.svg`/`supercopa.svg` ficam sem uso — Copa Argentina e Supercopa não são competições modeladas nos dados hoje, e mapear exigiria criar a competição inteira, não só a taça.
- **Limpeza**: `argentina_primera.json` tinha um campo solto `"midia": {"trofeu": "argentina apertura.svg"}` (fora do schema, nome de arquivo nem batia com o real) de uma tentativa manual anterior — removido, substituído pelos 3 campos `taca_*_url` corretos.
- **Validado**: 4 testes novos (`receitaArgentina`/`passosTurnoRetornoSomado` expõem os 2 títulos; com rating muito desigual o mesmo time lidera turno/returno/soma), 531 testes passando, `npx tsc --noEmit`/`cd web && npx tsc -b`/`cd web && npm run build` limpos, troféus conferidos visualmente no navegador (imagens reais, não ícone genérico nem SVG quebrado).

### 5.34. Cenários narrativos de "saída de campo" agora encerram a participação de verdade (implementado)

Usuário colou um skeleton próprio de "match engine" (`MatchState`/`PlayerMatchState`) propondo bloquear gol/assistência creditados a um jogador depois que ele sai de campo. Investigação: **cartão vermelho e lesão já tinham exatamente essa garantia** (`simulation/match.ts` `incidenteEncerraParticipacao` + `aindaEmCampo` em `simularPartida`; `jogadorAindaEmCampo` em `simulation/live-match.ts` `jogarPartidaAoVivo`) — colar o skeleton ali teria duplicado lógica já correta, com tipos incompatíveis com o resto do motor. Mas achei o bug real, só que em outro lugar: cenários narrativos de `progression/match-events.ts` que dizem "você foi substituído"/"você pediu pra sair"/"você foi expulso" (`tirado_no_intervalo`, `pedir_substituicao` em "Cãibra no fim do jogo", `pedir_substituicao_preventiva`/a expulsão de "Quase expulso em jogo tenso") não tinham NENHUM efeito mecânico — a narrativa dizia uma coisa, o motor simulado continuava gerando (e creditando) chances pro jogador no resto da partida como se nada tivesse acontecido.

- **Fix genérico, reaproveitável**: `ImpactoCarreira` (`progression/scenarios.ts`) ganhou `encerraParticipacaoNaPartida?: boolean`. `jogarPartidaAoVivo` liga isso no MESMO `jogadorAindaEmCampo` que já existia pra cartão vermelho/lesão — a partir daí nenhuma chance (gol/assistência) é atribuída ao jogador, E nenhum NOVO evento de contexto é sorteado pro resto da partida (todo o catálogo é narrado em 1ª pessoa como se ele estivesse ativo — não faz sentido continuar depois que ele já saiu). Corrige de uma vez qualquer cenário atual OU futuro que narre saída de campo, sem precisar lembrar de checar um por um.
- **4 opções corrigidas no catálogo**: `tirado_no_intervalo` (as 2 opções — o técnico já decidiu, a escolha é só COMO você reage, não SE sai), `pedir_substituicao` (cãibra), `pedir_substituicao_preventiva` (quase expulsão, saída preventiva). A expulsão de verdade (`se_conter_no_limite`, resultado de 30% "o árbitro decide expulsar você mesmo com a contenção") ganhou também `foraDeCombate: { motivo: "suspensao", partidasRestantes: 1 }` — mesma consequência de partida futura que um cartão vermelho automático já dava, que essa até então não tinha (era só narrativa solta).
- **Validado**: teste estatístico novo em `tests/simulation/live-match.test.ts` (60 seeds com PRNG seedado, `onEvento` monta a timeline completa da partida) — em todo seed onde o cenário de saída realmente dispara, nenhum `chance_jogador` nem `evento_de_contexto` aparece depois dele na timeline. 533 testes passando, `npx tsc --noEmit`/`cd web && npx tsc -b` limpos.
